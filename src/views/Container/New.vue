...
export default {
  ...
  data() {
    return {
      style_form: [
        {
          col: "12",
          type: "text",
          label: this.$i18n.t("Container"),
          placeholder: this.$i18n.t("Container"),
          error: null,
          value_text: "name",
          value: "",
      ...
      card: {
        title: this.$i18n.t("create container"),
        loading: true,
        sub_title: this.$i18n.t("Container info"),
      },
    };
  },
  methods: {
    set_data() {
      ...
      this.$store.commit("SET_URL", "gis/containers/store-with-camps");
      this.$store.commit("SET_PARENT_PAGE", "ContainerPage");
    ...
    get_data() {
      ...
      return axios.get('gis/containers/get-data').then(
        (response) => {
          this.$store.commit("SET_CARD_LOADING", false);
  ...
  mounted() {
    this.set_data();
    this.get_data();
    document.title = this.$i18n.t("create container");
  },
};
</script>
