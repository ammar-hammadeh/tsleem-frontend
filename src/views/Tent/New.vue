...
export default {
  ...
  methods: {
    set_data() {
      ...
      this.$store.commit("SET_URL", "gis/tents/store-with-camps");
    ...
    get_data() {
      ...
      return axios.get('gis/tents/get-data').then(
        (response) => {
          this.$store.commit("SET_CARD_LOADING", false);
  ...
  mounted() {
    this.set_data();
    this.get_data();
    document.title = this.$i18n.t("create tent");
  },
};
</script>
